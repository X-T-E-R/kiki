...
// renewal, writer handoff after close, and cluster-wipe exclusion while a
// writer is held.
...
test('wipe refuses while any shard writer is held and proceeds once released', async () => {
  ...
  try {
    ...
    // The shard locks — not the parent directory — are what make a wipe
    // refuse: deleting the cluster under a live writer destroys its data.
    ...
    // The single-file shape, and the rename-aside's ownership boundary: a
    // release after the store dir was moved must never unlink the lock file
    // of whoever now owns the original path.
