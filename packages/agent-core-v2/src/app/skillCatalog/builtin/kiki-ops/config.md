...
Use the first line when it is non-empty; otherwise use the second line. In the rest of this skill, `<KIKI_HOME>` means that resolved root — **never assume `~/.kiki`**.

- **`config.toml`** — agent / runtime settings: `default_model`, `[subagent]` (`timeout_ms`), `providers`, `models`, `thinking`, `permission`, `hooks`, `loop_control`, `nb_search`, etc.
- **`tui.toml`** — terminal-UI / client preferences: `theme`, `[editor].command`, `[notifications]`, and `[status_line]`. These can usually also be changed with the interactive commands `/config`, `/theme`, `/editor`, which is easier — prefer pointing the user at those.
...
- Location: `<KIKI_HOME>/config.toml` or `<KIKI_HOME>/tui.toml`. For other scopes/files, defer to the official docs.
