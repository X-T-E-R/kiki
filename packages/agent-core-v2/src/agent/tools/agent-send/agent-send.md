...
- Any **direct** child of the current agent — including unnamed children from `Agent` and children from `AgentSwarm`. Grandchildren are not reachable; send from their parent instead.
- Identify the child by the stable `name` you passed to `Agent`, or by its agent id. Swarm children and anonymous `Agent` children have no name; use the agent id `Agent` / `AgentSwarm` returned.
- Names are unique within the session and come only from the `name` parameter of `Agent`. Do not invent names. If you do not know a valid name or agent id, call `AgentList` first.
