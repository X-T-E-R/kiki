  ...
  describe('tool-call deltas', () => {
    ...
    it('preserves indexed tool-call deltas through AgentLoopService protocol events', async () => {
      ...
      expect(protocolEvents(ctx, 'tool.call.delta').map((event) => event.args)).toEqual([
        { time: expect.any(Number), turnId: 0, step: 1, stepId: expect.any(String), toolCallId: 'call_lookup', name: 'Lookup', argumentsPart: undefined },
        { time: expect.any(Number), turnId: 0, step: 1, stepId: expect.any(String), toolCallId: 'call_lookup', name: 'Lookup', argumentsPart: '{"query"' },
        { time: expect.any(Number), turnId: 0, step: 1, stepId: expect.any(String), toolCallId: 'call_lookup', name: 'Lookup', argumentsPart: ':"moon"}' },
      ]);
