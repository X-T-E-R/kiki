...
describe('agentRoots', () => {
  ...
  describe('projectRoots', () => {
    it('resolves the brand .kiki/agents directory at the .git root', async () => {
      await markGitRoot();
      ...
      expect(
        roots.some((r) => r.path.endsWith('.kiki/agents') && r.source === 'project'),
      ).toBe(true);
    });
    ...
    it('falls back to the generic .agents/agents directory', async () => {
      ...
      expect(roots.some((r) => r.path.endsWith('.kiki/agents'))).toBe(false);
    });
    ...
    it('orders the brand directory before the generic directory', async () => {
      ...
      const roots = await projectAgentRoots(hostFs, root);
      const brandIdx = roots.findIndex((r) => r.path.endsWith('.kiki/agents'));
