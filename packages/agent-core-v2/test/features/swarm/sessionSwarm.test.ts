...
describe('SessionSwarmService metadata compatibility', () => {
  ...
  it('keeps fixed resumed children on their own recorded model', async () => {
    agents['agent-existing'] = {
      labels: { parentAgentId: 'main', subagentBindingMode: 'fixed' },
    };
  ...
  it('refreshes inherited children from the caller before a warm resume', async () => {
    ...
    const child = agentHandle('agent-inherited', lifecycle, eventBus, {
      profileName: 'explore',
      modelAlias: 'stale-model',
      thinkingLevel: 'low',
    });
    handles.set('agent-inherited', child);
    const service = ix.get(ISessionSwarmService);

    await expect(
      service.run({
        callerAgentId: 'main',
        tasks: [resumeSessionTask('agent-inherited')],
      }),
...
function profileService(data: ProfileData): IAgentProfileService {
  ...
  return {
    ...
    setThinking: (level: string) => {
      current = { ...current, thinkingLevel: level as ProfileData['thinkingLevel'] };
    },
    republishStatus: () => {},
