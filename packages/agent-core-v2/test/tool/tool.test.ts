...
describe('SubagentToolInputSchema', () => {
  ...
  it.each([
    ...
    expect(() =>
      SubagentToolInputSchema.parse({ prompt: 'Investigate', name }),
    ).toThrow(/name/);
  });

  it('rejects name together with agent', () => {
    expect(() =>
      SubagentToolInputSchema.parse({
        prompt: 'Continue',
        agent: 'auth_probe',
...
describe('AgentRun tool execution contract', () => {
  ...
  it('uses the resumed agent profile in the activity description', async () => {
    ...
    if (execution.isError === true) throw new Error('expected runnable execution');
    expect(execution.description).toBe('Launching explore agent: Continue');
  ...
  it('returns an error when continuing with a profile', async () => {
    ...
    expect(result).toMatchObject({
      isError: true,
      output: 'Cannot set profile when continuing an existing agent. Pass only agent with the name or agent id.',
    });
    expect(lifecycle.run).not.toHaveBeenCalled();
  });
  ...
  it('resumes a foreground subagent when agent is provided', async () => {
    const lifecycle = createAgentLifecycleStub({
  ...
  it('steers the AI away from waiting and gives a resume hint on background launch', async () => {
    ...
    expect(result.output).toContain('AgentRun(agent="agent-child"');
  ...
  it('returns the spawned agent id when a foreground subagent times out', async () => {
    ...
    expect(result.output).toContain('resume_hint:');
    expect(result.output).toContain('AgentRun(agent="agent-child", prompt="continue")');
...
describe('Agent tools', () => {
  ...
  describe('registered user tools', () => {
    ...
    it('routes registered user tools through tool.call request/response', async () => {
      ...
      expect(ctx.lastLlmInput()).toMatchInlineSnapshot(`
        system: <system-prompt>
        tools: AgentList, AgentRun, AgentSend, AgentSwarm, AskUserQuestion, Bash, CreateGoal, Edit, EnterPlanMode, ExitPlanMode, FetchURL, GetGoal, Glob, Grep, list_threads, Lookup, Read, read_thread, send_message_to_thread, SetGoalBudget, Skill, TaskList, TaskOutput, TaskStop, TaskWait, TodoList, UpdateGoal, wait_threads, Write
        messages:
    ...
    `);
      ...
      `);
      expect(ctx.lastLlmInput()).toMatchInlineSnapshot(`
        tools: AgentList, AgentRun, AgentSend, AgentSwarm, AskUserQuestion, Bash, CreateGoal, Edit, EnterPlanMode, ExitPlanMode, FetchURL, GetGoal, Glob, Grep, list_threads, Read, read_thread, send_message_to_thread, SetGoalBudget, Skill, TaskList, TaskOutput, TaskStop, TaskWait, TodoList, UpdateGoal, wait_threads, Write
        messages:
          <last>
