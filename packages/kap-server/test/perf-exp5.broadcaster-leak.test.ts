...
function makeCore(host: FakeSessionHost): Scope {
  ...
  const handler = {
    ...
    accessor: {
      get: (token: unknown) => (token === ISessionLifecycleService ? lifecycle : undefined),
    },
    dispose: () => {},
  };
...
describe('perf experiment 5: SessionEventBroadcaster close retention', () => {
  it(
    'L1 records retained states, tails, target slots, listener cleanup, and heap growth',
    async () => {
      ...
      try {
        ...
        expect(statesOf(harness.broadcaster).size).toBe(20);
        expect(rows.map((row) => row.sessionsSize)).toEqual(
          Array.from({ length: 20 }, (_, index) => index + 1),
        );
        expect(rows.at(-1)?.tailEntries).toBe(20_000);
        expect(rows.at(-1)?.targetSlots).toBe(20);
        expect(rows.at(-1)?.uniqueTargets).toBe(1);
        expect(rows.at(-1)?.agentDisposableSlots).toBe(0);
  ...
  it(
    'L2 compares local agent dispatch and global allTargets scanning at 0 vs 20 closed sessions',
    async () => {
      ...
      try {
        ...
        expect(statesOf(retained.broadcaster).size).toBe(21);
        ...
        expect(retainedGlobal.scannedTargetSlots).toBe(21);
