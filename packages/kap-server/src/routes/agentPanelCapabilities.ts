...
export async function livePanelCapabilities(agent: IAgentScopeHandle): Promise<Pick<AgentCapabilitiesResponse, 'profile' | 'tools' | 'skills'>> {
  ...
  return {
    profile: {
      ...
      route: data.routeId, model: data.modelAlias, thinking_effort: data.effectiveThinkingLevel ?? data.thinkingLevel,
      thinking_effort_source: data.thinkingEffortSource,
      route_detached: data.routeDetached === true ? true : undefined,
      ...
      subagent_policy: data.subagentPolicy ?? 'advisory',
