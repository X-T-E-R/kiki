...
进入你的项目目录并运行 `kiki`，启动 daemon 支持的交互式界面：
...
从源码开发时，请在仓库根目录运行 `pnpm dev:cli`。要在不进入交互界面的情况下执行单条指令，使用 `-p`：
...
Kiki 会自动调用文件读取、搜索等工具浏览相关内容，然后再回答。默认情况下，只读操作自动执行、无需确认；修改文件或运行 Shell 命令的操作会先请求你的确认。
