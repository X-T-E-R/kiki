...
| `deny_models` | 否 | 该 role 的模型禁止列表，写法与 `allowed_models` 相同。与机器级 `[subagent].deny_models` 取并集；禁止名单始终优先于 `allowed_models`。Route sidecar 不能声明此字段 |
