...
Kimi Code CLI 通过环境变量控制少数运行时行为——迁移数据目录，以及不改配置文件临时切换模型。
...
| `KIMI_CODE_PLUGIN_MARKETPLACE_URL` | 设置 `/plugins` 加载的 plugin marketplace JSON，优先级高于 `[plugins] marketplace_url` | `http://` 或 `https://` URL、`file://` URL 或本地路径；未设置或留空时不加载远程目录 |
...
`[subagent] default_model` 与 `default_effort` 没有对应的环境变量。它们仍接受写在 `config.toml` 里，但 `AgentRun` / `AgentSwarm` 目前不会读。
