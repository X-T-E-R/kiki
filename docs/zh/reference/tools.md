...
与 MCP 工具相比，内置工具由运行时直接管理，生命周期与会话绑定，无需外部进程。两者都遵循统一的审批机制：**只读类工具**（如 `Read`、`Grep`、`Glob`）默认自动放行，**执行类工具**（如 `Bash`）默认需要用户审批。文件写入遵循工作区信任模型：在已信任的工作目录内，落在该目录下的 `Write` / `Edit` 不再逐项询问；工作区外的写入和敏感文件访问会被拦截或需要审批。YOLO 模式下普通工具调用的审批会被跳过，但 Plan 模式下的退出审批不受影响。
