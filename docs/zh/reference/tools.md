...
**`WebSearch`** 接受 `query`（搜索词），并使用 `[nb_search.defaults]` 配置的默认 lane。未配置 `search_lane` 时，工具会 fail-closed 并返回不可用错误。
