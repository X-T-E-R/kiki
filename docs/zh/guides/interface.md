...
在 subagent 的输入框中，停止请求尚未返回时，停止按钮会暂时禁用。若请求失败，错误提示会说明原因，按钮恢复可用后可重试。停止一次运行不会清空该 subagent 队列中其他待发送的消息。

## 审批

运行 Shell 命令的操作会以审批请求的形式出现在时间线中。每个请求在执行前会列出操作内容；你可以批准一次或批准整个会话。只读操作默认自动执行。文件写入遵循工作区信任模型：在已信任的工作目录内，落在该目录下的 `Write` / `Edit` 不再逐项询问；工作区外写入需要审批；访问敏感文件在 manual 模式询问用户、Auto 模式拒绝、YOLO 模式免询问（显式 deny 仍优先）。用 `Esc` 中断的工具调用在执行前停止。
