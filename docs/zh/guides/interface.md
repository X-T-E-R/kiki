...
运行 Shell 命令的操作会以审批请求的形式出现在时间线中。每个请求在执行前会列出操作内容；你可以批准一次或批准整个会话。只读操作默认自动执行。文件写入遵循工作区信任模型：在已信任的工作目录内，落在该目录下的 `Write` / `Edit` 不再逐项询问；工作区外的写入和敏感文件访问会被拦截或需要确认。用 `Esc` 中断的工具调用在执行前停止。
