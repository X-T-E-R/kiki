...
- **`model_alias`**：`AgentRun`、`AgentSwarm`、Agent 文件、profile route 与 caller lease 使用的已配置模型选择器。
...
恢复或重试的 subagent 保持已持久化的模型与 effort 绑定。`AgentRun` 使用 `resume` 时拒绝绑定字段；混合 `AgentSwarm` 调用只把这些字段应用于基于 item 的新派生项。
...
Route 声明的 `model_alias` 对自动派发锁定。`AgentRun` 与 `AgentSwarm` 可以省略它，或重复同一个解析后模型；冲突值会被拒绝。Role 级 `allowed_models`、`deny_models` 与机器级 `[subagent].deny_models` 只能继续收紧允许集合。
...
- **派发 schema**：`packages/agent-core-v2/src/agent/tools/agent/agent.ts` 和 `packages/agent-core-v2/src/features/swarm/tools/agent-swarm/agent-swarm.ts`
