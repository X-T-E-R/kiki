...
卡片会逐页显示；同步期间，计数和搜索结果只覆盖当前已加载的卡片。切换工作区范围或关闭看板会取消后续分页加载。在新建卡片、卡片详情或确认弹窗中，`Tab` 只在最上层弹窗内移动，`Esc` 只关闭该弹窗，并将焦点归还给打开它的控件。保存或删除请求尚未完成时，不能关闭对应弹窗。
...
在桌面版或浏览器的「新会话」页面，可以使用已有工作区、填写项目的绝对目录，或选择「自动创建工作区」。还没有已注册工作区时，默认使用自动创建。首次发送时，Kiki 会在 `$KIKI_HOME/workspaces/`（默认 `~/.kiki/workspaces/`）下新建目录，注册为工作区，并在其中开启会话。如果明确选中的工作区后来被删除，该选择会保持无效，直到重新选择其他工作区或自动创建；Kiki 不会悄悄换用其他已有工作区。目录结构和清理注意事项见[数据路径](../configuration/data-locations.md#目录结构)。
