...
服务在前台运行，按 `Ctrl-C` 干净退出。`--host`、`--log-level` 等完整选项见 [kiki 命令参考](../reference/kimi-command.md#kiki-web)。
...
::: danger 警告
`--dangerous-bypass-auth` 会彻底关闭鉴权，任何能访问该端口的人都能控制你的会话、文件系统和 shell。仅在可信网络或自有鉴权代理之后使用，详见 [kiki 命令参考](../reference/kimi-command.md#kiki-web)。
:::
...
- [kiki 命令](../reference/kimi-command.md#kiki-web) — `kiki web` 的全部命令行选项
