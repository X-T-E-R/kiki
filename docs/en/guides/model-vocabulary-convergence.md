...
- **`model_alias`**: the configured-model selector used by `AgentRun`, `AgentSwarm`, Agent files, profile routes, and caller leases.
...
Resumed and retried subagents keep their persisted model and effort binding. `AgentRun` rejects binding fields on `resume`, while a mixed `AgentSwarm` call applies them only to new item-based spawns.
...
A route-declared `model_alias` is locked for automatic dispatch. `AgentRun` and `AgentSwarm` may omit it or repeat the same resolved model, but a conflicting value is rejected. Role-level `allowed_models` and `deny_models`, plus machine-level `[subagent].deny_models`, can only narrow the allowed set.
...
- **Dispatch schema**: `packages/agent-core-v2/src/agent/tools/agent/agent.ts` and `packages/agent-core-v2/src/features/swarm/tools/agent-swarm/agent-swarm.ts`
