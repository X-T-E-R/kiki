...
The server runs in the foreground; press `Ctrl-C` for a clean shutdown. For the full option list such as `--host` and `--log-level`, see the [kiki command reference](../reference/kimi-command.md#kiki-web).
...
::: danger
`--dangerous-bypass-auth` disables authentication entirely — anyone who can reach the port can control your sessions, file system, and shell. Only use it on trusted networks or behind your own authenticating proxy. See the [kiki command reference](../reference/kimi-command.md#kiki-web).
:::
...
- [kiki command](../reference/kimi-command.md#kiki-web) — all `kiki web` command-line options
