...
When both are present, the dispatch value wins. When neither names a model, the spawn fails with `model.not_configured`; a subagent never inherits its caller's model and never falls back to a default. Unknown aliases and models rejected by machine-level or role-level constraints also fail before the child starts.
...
A route-declared `model_alias` is locked for automatic dispatch: the caller may omit it or repeat the same resolved model; conflicting values are rejected. Role-level `allowed_models` / `deny_models` and machine-level `[subagent].deny_models` can only narrow the permitted set, never widen it.
