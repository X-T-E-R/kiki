...
`kimi web` binds to local loopback only by default and prints the bearer token in the startup banner; the Kiki GUI authenticates automatically via the `#token=` URL fragment.
...
- **WARN**: The frontmatter contains an unknown key, the file shares a name with a built-in profile without `override: true`, or the `service_tier` field conflicts with `request_params.service_tier` (the nested value is ignored).
