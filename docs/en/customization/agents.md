...
`AgentRun` launches a new child or continues an existing one. Pass `name` when you expect to address the same child again; names must match `^[a-z0-9_]+$`, cannot be `root`, and stay unique for the session. Continue a child with `agent` set to that name or its agent id — do not also pass `name`, `profile`, `route`, `model`, `model_alias`, or `effort`. The UI label is derived from `name` or the first line of `prompt`; there is no `description` parameter.
...
`AgentSwarm` keeps its name. New item-based spawns take `profile` (defaults to `coder`) and `effort`; it still requires `description`.
