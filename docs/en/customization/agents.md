...
Peer-thread communication lets the main Agent coordinate existing Kiki sessions on the same local host, including sessions in other workspaces. It is separate from the child-agent tools above and is disabled by default. After opting in, the four tools `ThreadList`, `ThreadRead`, `ThreadSend`, and `ThreadWait` appear only on a session's main Agent, not its sub-agents. To create an independent session, main agents can use [`ThreadCreate`](../reference/tools.md#collaboration-tools) without enabling peer-thread communication.
...
::: warning Trust model
Agent files are prompt configuration, and project-level files come from the repository itself — including repositories you have just cloned and do not trust yet. A project-scoped file can take over a built-in agent entirely: a file named `agent.md` can replace the **default main agent's whole system prompt**, and `general.md` can replace the default subagent type. This does not require `override: true`. Unlike `AGENTS.md` content — which is injected into the prompt as reference data — an override file *is* the system prompt, and omitting `tools` adds no profile allowlist beyond the applicable runtime policy. Review `.kiki/agents/` and `.agents/agents/` in unfamiliar repositories with the same caution you would apply to scripts, before running Kiki inside them.
:::
...
| `model_alias` | no | Exact, case-sensitive alias from `[models]`, or `inherit` to explicitly bind a subagent to its caller's current model. A profile without a pin needs an explicit `model_alias` at dispatch; omitting it does not inherit. A main-agent profile cannot use `inherit` because it has no caller |
...
| `tools` | no | Allowlist of tool names such as `Read` or `Bash`; MCP tools are matched with globs such as `mcp__github__*`. Accepts a YAML list or a comma-separated string (`tools: Read, Grep`). Omitting it or using a lone `*` adds no profile allowlist; an empty list (`tools: []`) disables all tools. [Subagent defaults](../configuration/config-files.md#subagent) and other policy limits still apply; board tools need an exact entry or server opt-in |
...
| `subagents` | no | Child profile names this agent may dispatch, with the same syntax as `tools` (YAML list or comma-separated string). A declared list on a subagent is strict by default: `subagents: []` prohibits all new child dispatches, while listed names are permitted. Omit the field or use a lone `*` for no restriction |
| `subagent_policy` | no | `strict` enforces a declared `subagents` list; `advisory` permits other targets but records a recommendation deviation. An explicit profile value overrides the configured default |
...
Dispatch policy defaults are independent: `[subagent] main_dispatch_policy = "advisory"` applies to the main agent and `[subagent] subagent_dispatch_policy = "strict"` applies to subagents with a declared `subagents` list. Both accept `advisory` or `strict` and can be changed in Settings → Agents. Subagents with no declared list remain advisory regardless of that second default. The capability panel labels targets as preferred, allowed but not recommended, or blocked; `AgentRun` lists only permitted targets and highlights preferred ones. Model and thinking-effort recommendations remain advisory independently of dispatch policy.
...
`allowed_models`, role `deny_models`, and `allowed_efforts` shape recommendations and produce advisories when the effective binding deviates. They do not expand or narrow the machine permission boundary. Machine `[subagent].deny_models` always wins, even when the role recommends the same alias; route sidecars cannot declare the role list fields.
...
```yaml
# Recommend one alias for this role:
allowed_models: [fast-model]
```

Pair the recommendation with the `model_alias` you want as the default. A profile that declares `allowed_models` but no `model_alias` still loads with a warning, and a dispatch that names no model fails closed because recommendations do not select a model.
...
A newly spawned subagent binds its model from exactly two sources: the `model_alias` tool parameter, or the `model_alias` pin on the effective profile, route, or caller lease. The dispatch wins when both are present. Without either source, the spawn fails with `model.not_configured` and no child is created: the caller's model and `default_model` are never silent fallbacks. Set `model_alias: inherit` in a profile, route, or caller lease, or pass `model_alias: "inherit"` to `AgentRun`, to explicitly bind the caller's current resolved model. In that case the caller's effective thinking effort follows too, unless an explicit tool `effort` or applicable `thinking_effort` pin on the profile, route, caller lease, or matching `model_profiles` entry takes priority. For other model selections, effort follows the existing precedence: explicit tool `effort` → matching `model_profiles` effort → profile `thinking_effort` when the bound model matches the profile pin (canonical identity) → the bound model's own default. An unknown concrete alias is an error whether it came from the dispatch or from a profile pin.

On `AgentRun` resume the saved binding is kept when you omit both `model_alias` and `effort`. A `model_alias` that resolves to the same canonical model is a no-op. Changing only `effort` applies the new value to the next idle run and keeps the saved model. Changing `model_alias` to a different canonical model requires `allow_model_change: true`; when `effort` is also omitted in that case, the target model's own default effort is re-resolved from scratch — the previous effort is not carried over. The explicit `model_alias: "inherit"` exception resolves against the *current* caller binding on resume as well; unless an applicable saved effort pin or explicit `effort` wins, it also uses the caller's effective effort. Role guidance and saved route/caller-lease pins produce advisories instead of blocking a runnable resume. An explicit effort the provider cannot honor, machine model denial, executor thread-binding restrictions, and admission consistency checks remain hard errors.

Omit `model_alias` and `effort` to use the selected target's defaults when launching a new child. `AgentRun` lists configured models recommended by each target's effective profile, lease, route, and model guidance. A model listed for another target is not a recommendation for the current one, but an explicit executable override is accepted and diagnosed. Pair `allowed_models` with a default `model_alias` to publish a preferred pool; `model_profiles` adds per-model recommendations. Route tier overrides, including `service_tier: null`, do not clear a configured model-level tier.

Subagent model governance compares canonical model identities after resolving `[models]` aliases. Machine `[subagent].deny_models` is the hard model policy and rejects listed models at every subagent entry. Role `allowed_models` / `deny_models`, model-profile effort lists, route defaults, and caller-lease pins are advisory; deviations are retained in the child binding and summarized in the parent `AgentRun` result. See the [configuration reference](../configuration/config-files.md#subagent) for fields and validation rules.
...
In print mode, an explicit `--model` takes precedence over the selected profile's `model_alias`. Without `--model`, the engine uses the profile pin first and `default_model` only when no profile model is set. A pinned profile therefore works without a global default; unlike main agents, subagents never fall back to `default_model`. A main-agent profile cannot pin `model_alias: inherit` even if `default_model` or `--model` is set: it has no caller to follow.
