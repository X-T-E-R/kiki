...
| `KIMI_CODE_EXPERIMENTAL_WAIT_FOR` | Whether the model is given the `TaskWait` tool, which waits for background tasks inside the current turn instead of ending it (enabled by default). The variable and its flag id keep the older `wait_for` spelling so existing settings keep working, even though the tool is now named `TaskWait` | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
