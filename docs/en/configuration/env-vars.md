...
| `KIMI_CODE_EXPERIMENTAL_AGENT_COLLABORATION` | Enable the six-tool Codex-style named-agent adapter, including `send_message`; `[agents] enabled = false` still disables the adapter | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
