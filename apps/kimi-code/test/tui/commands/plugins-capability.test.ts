...
import { log } from '@kiki/node-sdk';
import { resetCapabilitiesCache, setCapabilities, type Component } from '@kiki/pi-tui';
...
describe('plugins command capability surface', () => {
  ...
  it('removePlugin notes that capability runtimes are left untouched', async () => {
    ...
    expect(statuses.some((s) => s.includes('Restart Kiki before reinstalling'))).toBe(true);
  ...
  it('treats only the source checkout and its dev server as injectable', () => {
    expect(isDefaultMarketplaceCatalog(undefined, undefined, {})).toBe(true);
    ...
    expect(
      isDefaultMarketplaceCatalog(
        undefined,
        'https://config.test/marketplace.json',
        {},
      ),
    ).toBe(false);
    expect(
