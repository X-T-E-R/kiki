...
describe('AgentSwarmProgressEstimator', () => {
  it('reuses the completed-sample prior across every estimate until a mutation invalidates it', () => {
    const estimator = new AgentSwarmProgressEstimator();
    estimator.markStarted('completed', 0);
    for (let index = 0; index < 10; index += 1) {
      estimator.recordToolCall({
        memberKey: 'completed',
        toolCallId: `done-${String(index)}`,
        nowMs: 1_000 + index * 1_000,
      });
    }
    estimator.markCompleted('completed', 40_000);
    const running = Array.from({ length: 20 }, (_, index) => `run-${String(index)}`);
    for (const memberKey of running) estimator.markStarted(memberKey, 0);

    const spy = vi.spyOn(
      estimator as unknown as { computePrior: () => unknown },
      'computePrior',
    );
    try {
      for (const memberKey of running) {
        estimator.estimate({ memberKey, phase: 'running', capacityTicks: 56, nowMs: 20_000 });
      }
      expect(spy).toHaveBeenCalledTimes(1);

      const repeated = estimator.estimateAll(
        running.map((memberKey) => ({
          memberKey,
      ...
      expect(repeated.size).toBe(running.length);
      expect(spy).toHaveBeenCalledTimes(1);

      estimator.markCompleted(running[0]!, 25_000);
      estimator.estimate({
        memberKey: running[1]!,
        phase: 'running',
        capacityTicks: 56,
