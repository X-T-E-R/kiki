...
describe('targetTriple', () => {
  ...
  it('honors KIKI_BUILD_TARGET override', () => {
    expect(
      targetTriple({
        platform: 'darwin',
        arch: 'arm64',
        env: { KIKI_BUILD_TARGET: 'linux-arm64' },
      }),
    ).toBe('linux-arm64');
  });
});
...
describe('executableName', () => {
  it('returns kiki.exe on win32', () => {
    expect(executableName('win32')).toBe('kiki.exe');
  });

  it('returns kiki on other platforms', () => {
    expect(executableName('darwin')).toBe('kiki');
    expect(executableName('linux')).toBe('kiki');
  });
});
