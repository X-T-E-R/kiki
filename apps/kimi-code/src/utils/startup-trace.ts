...
//
// Debug-only startup phase tracer, enabled with KIKI_STARTUP_TRACE=1.
// Each call appends one `<elapsed-ms> <label>` line to the trace file
// (default /tmp/kimi-startup-trace.log, override with
// KIKI_STARTUP_TRACE_LOG=<path>), so a slow or BLOCKING startup phase
...
const enabled = process.env['KIKI_STARTUP_TRACE'] !== undefined && process.env['KIKI_STARTUP_TRACE'] !== '';
const logPath = process.env['KIKI_STARTUP_TRACE_LOG'] ?? '/tmp/kimi-startup-trace.log';
