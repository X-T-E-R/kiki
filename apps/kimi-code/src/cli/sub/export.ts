...
export function registerExportCommand(parent: Command, deps?: Partial<ExportDeps>): void {
  parent
    ...
    .option(
      ...
      'Skip bundling the active global diagnostic log (~/.kiki/logs/kimi-code.log, not rotated .1 files). By default the global log is included.',
    )
...
function createDefaultExportDeps(overrides: Partial<ExportDeps> = {}): ExportDeps & {
  ...
  return {
    ...
    exportSession: overrides.exportSession ?? ((input: ExportSessionInput) => getHarness().exportSession(input)),
