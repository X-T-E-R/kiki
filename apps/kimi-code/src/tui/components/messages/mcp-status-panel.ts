import type { McpServerInfo } from '@kiki/node-sdk';
...
export function buildMcpStatusReportLines(options: McpStatusReportOptions): string[] {
  ...
  if (servers.length === 0) {
    lines.push(muted('  No MCP servers configured. Run /kiki-ops to add one.'));
    return lines;
  }
  ...
  for (const server of servers) {
    ...
    if (server.status === 'needs-auth') {
      lines.push(`    ${muted('action:')} ${value(`run /kiki-ops help me log in to MCP ${server.name}`)}`);
    }
  }

  lines.push('');
  lines.push(`  ${value(buildSummary(servers))}`);
  lines.push(`  ${muted('Configure with')} ${value('/kiki-ops')}`);
