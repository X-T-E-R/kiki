...
`/api/v1/ws` is an incremental channel (`transcript.ops`, grade `block` — the cheapest grade that still carries whole-state frame upserts, dropping per-token `append` frames). `transcript.reset` is applied in place through the package reset reducer (coverage-aware: a tail snapshot keeps already-loaded older turns and unchanged node identity). The channel tracks the per-agent `{epoch?, seq}` cursor: `subscribe_v2` carries grades plus `transcript_since`, reconnect / `resync_required` / append-offset gap first catch up (`fetchTranscriptOps` → `GET .../transcript/ops?since_seq=&epoch=`), and only `complete: false` falls back to a baseline REST reset. Grade-filtered seq jumps are not gaps. Convergence reuses `@moonshot-ai/transcript`'s L2 reducer (`src/transcript/`: REST/WS clients + store; the data model and reducer come from the package, nothing is re-implemented locally).
