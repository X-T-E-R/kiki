 ...
 * Host cannot discover the developer's real shared Kiki home.
...
beforeEach(() => {
  ...
  vscodeTest.runTests.mockImplementation(async (options) => {
    await writeFile(
      options.extensionTestsEnv.KIKI_VSCODE_SMOKE_REPORT,
...
describe('installed VSIX Extension Host smoke', () => {
  ...
  it('gives the harness separate Kiki and operating-system homes before activation', async () => {
    const fixture = await makeFixture();
    ...
    expect(env.KIKI_HOME).not.toBe(env.KIKI_VSCODE_SMOKE_OS_HOME);
    expect(env.KIKI_VSCODE_SMOKE_OS_HOME).toContain('os-home');
  });
...
async function makeFixture(): Promise<{ vsixPath: string; cachePath: string }> {
  const root = await mkdtemp(join(tmpdir(), 'kiki-extension-host-smoke-'));
  tempDirs.push(root);
  const cachePath = join(root, 'cache');
  const vsixPath = join(root, 'kiki-test.vsix');
